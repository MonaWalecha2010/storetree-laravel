<?php
    ...
    if(isset($main_person['pids'])){
        if($main_person['pids'][0]!=''){
            $u = \App\Models\FamilyTree::where('user_id', $main_person['user_id'])->first();
            // dd($main_person);
            $u_partner= \App\Models\FamilyTree::where('id', $u->pid)->first();
            
            if($main_person['gender']!= $u_partner->gender){
                foreach ($all_relations as $key => $value) {
    ...
    //  $connection_with_search_user = false;
    // if (Auth::check()) {
    //     foreach ($new_datas as $filter_data) {
    //         if ($filter_data['user_id'] == Auth::user()->id) {
    //             $connection_with_search_user = true;
    //         }
    //      }
    // }
            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="blog_filter_section">
                        <div class="blog_tittle_left">Family Tree Of {{$main_person['name']}}</div>
                                                        ...
                                                        <input type="text" hidden
                                                            ...
                                                            <div class="col-md-5 col-sm-5 col-xs-5">
                                                                 <div class="pr_photo" style="border-radius: 0;">
                                                                    <img src="http://127.0.0.1:8000/images/frontend/{{ $main_person['gender']=='male'?'photo_male.png':'photo_female.png'}}"
...
$('#closeButton2').on('click',function(){
 let vid = document.getElementById("friend_video");
     vid.pause();
});
...
console.log(gotItem.photo);
    document.getElementById('user_ps_name').innerText = gotItem.name;
    document.getElementById('user_ps_gender').innerText = gotItem.gender.charAt().toUpperCase() + gotItem.gender.slice(1);

  
              $('#pop_img').attr('src', gotItem.photo);
       

           // alert($('#user_p_name').html());


        // alert(gotItem.gender);
        // document.getElementById('user_p_gender').innerText = gotItem.gender.charAt().toUpperCase() + gotItem.gender
        // .slice(1);
