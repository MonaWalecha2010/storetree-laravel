    ...
    <div class="common_section story_section">
        <div class="container">
            <div class="row">
                <div class="col-xs-12">
                    <div class="pp_top_section">
                        ...
                        <p>Based on your <strong>Questions</strong> Click the boxes next to the questions you would like to select (total to equal selected above)</p>
                    </div>
                </div>
            </div>
            {!! Form::open(['method'=>'POST', 'action'=>'frontend\CreateStoryController@edit_step2Store']) !!}
            <div class="row choose_select">
                ...
                <div class="col-xs-12 col-sm-6 col-md-6">
                    <div class="choose_select_common">
                        <h3>{{ $category->title }}</h3>
                         @if($category->sort == 6)
                            <span class="own_question_note" style="color:red; font-size:13px">Note :  When RECORDING, Please state your question before giving your answer/story.</span>
                            @endif
                        <ul>
                            @foreach($category->questions->where('status', 1) as $question)
                            <li>
                                <div class="av_check">
                                    <input name="questions[]" value="{{ $question->id }}" id="ct1_{{ $question->id }}" type="checkbox" class="question-check" onchange="QuestionPlanCheck('{{ $question->id }}')"  @if(in_array($question->id, $question_ids)) checked @endif>
                ...
                @endforeach
                <input hidden name="story_id" value="{{ $story_id }}" id="story_{{$story_id}}" type="text" >
                                <input hidden name="package" value="{{ $package }}" id="story_package" type="text" >
            ...
            <div class="row">
                <div class="col-xs-12 padding_cs_1">
                    <div class="step_bottom_section">
                        ...
                        <div class="common_btn" style="display: contents"><a href="{{ route('view-story') }}">Back</a></div><!--step_back-->
...
<script type="text/javascript">
  function QuestionPlanCheck(id) {
    ...
    if (package == 3) {
        allowed_questions = 10 ; 
    }


        var length = $(".question-check:checked").length;
        if(length >allowed_questions){
